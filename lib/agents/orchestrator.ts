...
Return exactly ONE word from the above list.`;
  ...
  try {
    switch (agentUsed) {
      case 'teacher':
        const teacherContext = await runRetrievalAgent(input, undefined, context?.userClasses);
      ...
      case 'student':
        ...
        const studentContext = await runRetrievalAgent(input, context?.subject, context?.userClasses);
      ...
      case 'recommendation':
        ...
        break;
      case 'retrieval':
        responseData = await runRetrievalAgent(input, context?.subject, context?.userClasses);
        break;
      case 'code':
      ...
      case 'analytics':
        ...
        break;
      default:
        const fallbackContext = await runRetrievalAgent(input, undefined, context?.userClasses);
