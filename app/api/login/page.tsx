    ...
    try {
      const res = await fetch('/api/signup', {
        ...
        body: JSON.stringify({ 
          ...
          classes: role === 'Teacher' ? classes.split(',').map(c=>c.trim()).filter(Boolean) : [classes.trim()].filter(Boolean)
        })
      })
      ...
      if (!res.ok) {
        ...
        } else if (res.status === 400 && typeof data.error === 'string' && data.error.includes('already exists')) {
        ...
        {activeTab === 'signup' && (
          <form className="space-y-5" onSubmit={handleSignUp}>
            ...
            {role === 'Teacher' && (
               <>
                  <div>
                     <label className="block text-sm font-semibold text-gray-700 mb-1.5">Subjects Handling</label>
                     <input name="subjects" type="text" required className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. Science, Mathematics" value={subjects} onChange={(e) => setSubjects(e.target.value)} />
                  </div>
                  <div>
                     <label className="block text-sm font-semibold text-gray-700 mb-1.5">Classes Handling</label>
                     <input name="classes" type="text" required className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. Grade 6, Grade 7" value={classes} onChange={(e) => setClasses(e.target.value)} />
                  </div>
               </>
            )}
            ...
            {role === 'Student' && (
               <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Class/Grade Information</label>
                  <input name="classes" type="text" required className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" placeholder="e.g. Grade 6" value={classes} onChange={(e) => setClasses(e.target.value)} />
               </div>
            )}
