...
export default function TeacherExamCreator() {
  ...
  const teacherClasses = session?.user?.classes || [];
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-4">
               ...
               <div>
                  ...
                  {teacherSubjects.length > 0 ? (
                    <select className="w-full border p-2 rounded-lg" value={exam.subject} onChange={e=>setExam({...exam, subject: e.target.value})}>
                      {teacherSubjects.map((s: string) => <option key={s} value={s}>{s}</option>)}
                    </select>
                  ) : (
                    <input className="w-full border p-2 rounded-lg" value={exam.subject} onChange={e=>setExam({...exam, subject: e.target.value})} />
                  )}
               </div>
               <div>
                  <label className="text-xs font-bold uppercase text-gray-500">Class</label>
                  {teacherClasses.length > 0 ? (
                    <select className="w-full border p-2 rounded-lg" value={exam.className} onChange={e=>setExam({...exam, className: e.target.value})}>
                      {teacherClasses.map((c: string) => <option key={c} value={c}>{c}</option>)}
                    </select>
                  ) : (
                    <input className="w-full border p-2 rounded-lg" value={exam.className} onChange={e=>setExam({...exam, className: e.target.value})} />
                  )}
               </div>
