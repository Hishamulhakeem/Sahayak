...
export default function GenerateNewContentPage() {
  ...
  useEffect(() => {
    if (teacherSubjects.length > 0 && !subject) setSubject(teacherSubjects[0])
    if (teacherClasses.length > 0 && !className) setClassName(teacherClasses[0])
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/content-generations', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: `${className} ${subject}: ${topic} ${contentType === 'worksheet' ? 'Assignment' : 'Notes'}`,
          subject,
          topic,
          className,
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="worksheet">Assignment / Worksheet</option>
                </select>
              </div>
            </div>
            <div>
