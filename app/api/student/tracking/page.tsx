            ...
            <div className="grid md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between col-span-1">
                <div>
                  ...
                  <h2 className="text-4xl font-black text-gray-900">{data?.classRank || 'N/A'}</h2>
                </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between col-span-1">
                <div>
                  ...
                  <h2 className="text-4xl font-black text-gray-900">{data?.avgQuizScore || 0}%</h2>
                </div>
                <div className="bg-green-100 p-2 rounded-lg text-green-600 flex items-center space-x-1 font-semibold text-sm">
                  <TrendingUp className="w-4 h-4" /> <span>Current</span>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between col-span-1">
                <div>
                  ...
                  <h2 className="text-4xl font-black text-gray-900">{data?.longestStreak || 0} <span className="text-lg text-gray-500">days</span></h2>
                </div>
              ...
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-green-100 bg-gradient-to-r from-transparent to-green-50/50">
                ...
                <div className="space-y-4">
                  {data?.strongestTopics?.length > 0 ? (
                    data.strongestTopics.map((topic: any, i: number) => (
                      <div key={i} className="flex justify-between items-center bg-white p-3 border rounded-xl">
                        <span className="font-semibold text-gray-700">{topic.name}</span>
                        <span className="bg-green-100 text-green-700 font-bold px-3 py-1 rounded-full text-sm">{topic.mastery}% Mastery</span>
                      </div>
                    ))
                  ) : (
                    <div className="bg-white p-6 rounded-xl border border-dashed border-gray-200 text-center">
                      <p className="text-gray-500 text-sm font-medium">No strengths recorded yet.</p>
                      <p className="text-[10px] text-gray-400 mt-1 uppercase">Complete lessons to see your mastery levels here.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white p-8 rounded-2xl shadow-sm border border-red-100 bg-gradient-to-r from-transparent to-red-50/50">
                ...
                <div className="space-y-4">
                  {data?.needsImprovement?.length > 0 ? (
                    data.needsImprovement.map((topic: any, i: number) => (
                      <div key={i} className="flex justify-between items-center bg-white p-3 border rounded-xl border-l-4 border-l-red-500">
                        <div>
                          <span className="font-semibold text-gray-700 block">{topic.name}</span>
                        </div>
                        <span className="bg-orange-100 text-orange-700 font-bold px-3 py-1 rounded-full text-sm">{topic.mastery}% Mastery</span>
                      </div>
                    ))
                  ) : (
                    <div className="bg-white p-6 rounded-xl border border-dashed border-gray-200 text-center">
                      <p className="text-gray-500 text-sm font-medium">No areas identified for improvement yet.</p>
                      <p className="text-[10px] text-gray-400 mt-1 uppercase">Take quizzes to help the AI map your learning path.</p>
                    </div>
                  )}
                </div>
                <button 
                  onClick={() => window.location.href = '/student/chat'}
                  className="w-full mt-6 bg-blue-600 text-white font-bold py-3 px-4 rounded-xl hover:bg-blue-700 shadow-md transition-all flex justify-center items-center"
                >
